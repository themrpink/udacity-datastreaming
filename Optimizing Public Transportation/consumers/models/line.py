...
class Line:
    ...
    def process_message(self, message):
        ...
        if "com.udacity.cta.stations.table" in message.topic():
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif "com.udacity.cta.stations": # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif "com.udacity.cta.stations.turnstile": # Set the conditional to the KSQL Turnstile Summary Topic
